...
class ModelExtensionModuleAutoFeatured extends Model {
	...
	public function get_all() {
		$this->load->model('catalog/product');
		$get_cats = $this->db->query("SELECT category_id FROM " . DB_PREFIX . "category");
		foreach ($get_cats->rows as $value) {
			$cat_data[$value['category_id']] =  $value['category_id'];

			$data = array('filter_category_id' => $value['category_id'] );
			$result[$value['category_id']] = $this->model_catalog_product->getProducts($data);
			if (empty($result[$value['category_id']])) {
				unset($result[$value['category_id']]);
			}
	...
	public function getcats_desc($cat_id) {
		$get_cats = $this->db->query("SELECT name FROM " . DB_PREFIX . "category_description WHERE category_id = '" . (int)$cat_id . "'");
		return $get_cats->row['name'];
	}
}